(function () {
  ...
  function drawIidPlateFigure(target, repeated) {
    var card = makeStaticCard(
      ...
      repeated
        ? 'The open node is the latent parameter; the red node is an observed value repeated inside the plate for i = 1, ..., n.'
        : 'The parameter theta generates an observed sample x through the likelihood p(x | theta).'
    );
    var svg = createSvg(820, 300, '0 0 820 300');
    svg.setAttribute('aria-label', repeated ? 'Plate notation diagram for independent observations generated by theta.' : 'Graphical model diagram where theta generates x.');
    ...
    if (repeated) {
      [['x', 1, 330, 165], ['x', 2, 410, 165], ['...', '', 488, 166], ['x', 'n', 570, 165]].forEach(function (node, index) {
        if (node[0] === '...') {
          svg.appendChild(text(node[2] - 10, node[3] + 6, '...', 'diagram-paper-label diagram-copy'));
          return;
        }
        svg.appendChild(el('circle', { cx: node[2], cy: node[3], r: 22, class: 'diagram-paper-node diagram-paper-node--observed-red diagram-copy' }));
        svg.appendChild(mathText(node[2], node[3] + 6, node[0], node[1], 'diagram-paper-math diagram-paper-math--white diagram-copy'));
        svg.appendChild(el('path', { d: 'M160 70 C230 70 ' + (node[2] - 36) + ' 96 ' + node[2] + ' 136', class: 'diagram-paper-arrow diagram-paper-arrow--thin diagram-copy', 'marker-end': markerUrl }));
      });
      svg.appendChild(text(286, 74, 'same parameter, repeated likelihood terms', 'diagram-paper-small diagram-copy'));
      svg.appendChild(text(286, 238, 'P(theta, x1, ..., xn) = P(theta) * product_i P(x_i | theta)', 'diagram-paper-small diagram-copy'));
    } else {
      svg.appendChild(text(286, 78, 'prior: P(theta)', 'diagram-paper-small'));
      svg.appendChild(text(286, 128, 'likelihood: P(x | theta)', 'diagram-paper-small'));
      svg.appendChild(text(286, 196, 'joint: P(theta, x) = P(theta) P(x | theta)', 'diagram-paper-small'));
    }
